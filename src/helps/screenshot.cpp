        ...
        QDialog(parent)
{
    ...
    loadBackgroundPixmap(fullScreenShot(), 0, 0, width, height);
}
...
void ScreenShot::loadBackgroundPixmap(const QPixmap &bgPixmap, int x, int y, int width, int height)
{
    loadPixmap = bgPixmap;
    screenx = x;
    screeny = y;
...
void ScreenShot::mouseDoubleClickEvent(QMouseEvent *event)
{
    if (currentShotState == finishShot || currentShotState == finishMoveShot || currentShotState == finishControl) {
        emit finishPixmap(mShotPixmap); //当完成时发送finishPixmap信号
        accept();
    }
}
...
void ScreenShot::initFullScreenWidget()
{
...
QRect ScreenShot::getMoveAllSelectedRect()
{
    QRect result;
...
QRect ScreenShot::getMoveControlSelectedRect()
{
    int x, y, w, h;
...
QPixmap ScreenShot::windowScreenShot()
{
    if (QApplication::activeWindow()) {
        ...
        //        pixmap = QPixmap::grabWindow( QApplication::activeWindow()->winId());
...
void ScreenShot::partScreenShot(const QString &savePath)
{
